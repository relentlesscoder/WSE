...
class Evaluator {
	...
	private static Map<Integer, Double> getPrecisionMap(
	    HashMap<Integer, RelevancePair> relevanceJudgments,
		...
		for (int i = 0; i < K.length; i++) {
			...
			for (int j = 0; j < k; j++) {
				...
				if (relevanceJudgments.containsKey(docId)
				    && relevanceJudgments.get(docId).getBinaryRelevance() == 1) {
					rr++;
				}
			}
	...
	private static Map<Integer, Double> getRecallMap(
	    HashMap<Integer, RelevancePair> relevanceJudgments,
	    List<Integer> rankedList) {
		// <K, Recall>
	...
	private static Map<Integer, Double> getFMap(
	    Map<Integer, Double> precisionMap, Map<Integer, Double> recallMap) {
		// <K, F>
	...
	private static double getAveragePrecision(
	    HashMap<Integer, RelevancePair> relevanceJudgments,
	    List<Integer> rankedList) {
		...
		for (int i = 0; i < lengthOfRankedList; i++) {
			...
			if (relevanceJudgments.containsKey(docId)
			    && relevanceJudgments.get(docId).getBinaryRelevance() == 1.0) {
				rr += 1.0;
				ap += rr / (i + 1);
			}
		}
	...
	private static double getReciprocal(
	    HashMap<Integer, RelevancePair> relevanceJudgments,
	    List<Integer> rankedList) {
		double i = 0.0;
	...
	private static Map<Integer, Double> getDCG(
	    HashMap<Integer, RelevancePair> relevanceJudgments,
	    List<Integer> rankedList) {
		...
		for (int i = 0; i < K.length; i++) {
			...
			for (int j = 0; j < k; j++) {
				int docId = rankedList.get(j);
				if (relevanceJudgments.containsKey(docId)) {
					dcg += relevanceJudgments.get(docId).getCategoricalRelevance()
					    / Math.log(j + 2);
				}
			}
	...
	private static Map<Integer, Double> getIDCG(
	    HashMap<Integer, RelevancePair> relevanceJudgments,
	    List<Integer> rankedList) {
	...
	private static Map<Integer, Double> getNDCG(
	    HashMap<Integer, RelevancePair> relevanceJudgments,
	    List<Integer> rankedList) {
	...
	private static List<Double> getPrecisionAtStandardRecalls(
	    HashMap<Integer, RelevancePair> relevanceJudgments,
	    List<Integer> rankedList) {
		...
		Collections.sort(precisionRecallPairList, new Comparator<PRPair>() {
			...
			public int compare(PRPair o1, PRPair o2) {
				return o1.getRecall() > o2.getRecall() ? 1 : o1.getRecall() < o2
				    .getRecall() ? -1 : 0;
			}
		});
	...
	// categorical relevance pair>>
		...
		precisionAtStandardRecalls = getPrecisionAtStandardRecalls(
		    relevanceJudgments.get(query), rankedList);
		averagePrecision = getAveragePrecision(relevanceJudgments.get(query),
		    rankedList);
